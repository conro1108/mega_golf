...
import { loadBest, saveBestIfBetter, memoryStorage } from "./persistence";
...
describe("persistence", () => {
  ...
  it("treats corrupt JSON as no best run rather than throwing", () => {
    const storage = memoryStorage();
    storage.setItem("megagolf:best:Warm Up", "{not json");
    expect(loadBest(storage, "Warm Up")).toBeNull();
  });

  it("treats a value missing the expected shape as no best run", () => {
    const storage = memoryStorage();
    storage.setItem("megagolf:best:Warm Up", JSON.stringify({ strokes: 3 }));
    expect(loadBest(storage, "Warm Up")).toBeNull();
    storage.setItem("megagolf:best:Warm Up", JSON.stringify({ strokes: 3, shots: [{ angle: "x", power: 1 }] }));
    expect(loadBest(storage, "Warm Up")).toBeNull();
  });
});
